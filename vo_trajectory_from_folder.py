...
from Datasets.transformation import ses2poses_quat, kitti2tartan , pos_quats2SEs
...
if __name__ == '__main__':
   ...
   # testvo = TartanVO(args.model_name)
    ...
    poselist = np.loadtxt("../Archive/EuRoC/Trajectories/V_101.txt")
    testname = "results_of_V101"

    print(poselist.shape)
    
    model_gt_file =  np.loadtxt(args.pose_file,comments = '#',delimiter = ',')

    if(model_gt_file.shape[1] == 12):
       

        converted_gt_file = kitti2tartan(model_gt_file)

    elif(model_gt_file.shape[1] == 17):
        model_gt_file = model_gt_file[::10,1:8]
        model_gt_file = model_gt_file[:poselist.shape[0]]  #[:1890]
        


    print(model_gt_file.shape)

    # calculate ATE, RPE, KITTI-RPE
    if True:
        evaluator = TartanAirEvaluator()
        results = evaluator.evaluate_one_trajectory(model_gt_file, poselist, scale=True, kittitype=(datastr=='kitti'))
        if datastr=='euroc':
